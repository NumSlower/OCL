...
typedef struct OclStruct OclStruct;
typedef struct OclCell OclCell;
typedef struct OclFunction OclFunction;
...
typedef struct {
    ...
    union {
        ...
        char      char_val;
        OclArray *array_val;
        OclStruct *struct_val;
        OclFunction *function_val;
    } data;
} Value;
...
static inline Value value_struct(OclStruct *s) {
    if (s) ocl_struct_retain(s);
    return (Value){VALUE_STRUCT, true, {.struct_val = s}};
}

static inline Value value_function(OclFunction *fn) {
    if (fn) ocl_function_retain(fn);
    return (Value){VALUE_FUNCTION, true, {.function_val = fn}};
}
...
/* Optional allocation tracking for leak reporting (DEBUG + OCL_MEMTRACE=1). */
