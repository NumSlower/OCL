...
static void skip_whitespace(Lexer *l) {
    while (l->position < l->source_len) {
        ...
        if (c == ' ' || c == '\t' || c == '\r') advance(l);
            ...
            advance(l); advance(l);
            while (l->position < l->source_len) {
                if (cur(l) == '#' && peek(l) == '/') { advance(l); advance(l); break; }
                advance(l);
            }
        } else break;
    }
...
static Token make_token(Lexer *l, TokenType t, const char *lex) {
    ...
    tok.type = t; tok.lexeme = ocl_strdup(lex); tok.lexeme_length = strlen(lex);
...
static Token read_string(Lexer *l, TokenType t) {
    ...
    Token tok; memset(&tok, 0, sizeof(tok));
    tok.type = t; tok.location = (SourceLocation){start_line, start_col, l->filename};
    tok.value.string_value = ocl_strdup(buf); tok.value.string_length = len;
    tok.lexeme = ocl_strdup(buf); tok.lexeme_length = len;
    return tok;
}

static Token read_number(Lexer *l) {
    int start_line = l->line, start_col = l->column;
    char buf[64]; size_t len = 0; bool is_float = false;
    ...
    if (l->position < l->source_len && cur(l) == '.' && isdigit((unsigned char)peek(l))) {
        is_float = true; buf[len++] = '.'; advance(l);
        while (l->position < l->source_len && isdigit((unsigned char)cur(l)))
            { buf[len++] = cur(l); advance(l); }
    }
    buf[len] = '\0';
    ...
    tok.lexeme = ocl_strdup(buf); tok.lexeme_length = len;
    if (is_float) { tok.type = TOKEN_FLOAT; tok.value.float_value = strtod(buf, NULL); }
    else { tok.type = TOKEN_INT; tok.value.int_value = (int64_t)strtoll(buf, NULL, 10); }
    return tok;
}
...
static Token read_identifier(Lexer *l) {
    ...
    while (l->position < l->source_len) {
        char c = cur(l);
        if (isalnum((unsigned char)c) || c == '_') { buf[len++] = c; advance(l); }
        else break;
    }
    buf[len] = '\0';
...
Lexer *lexer_create(const char *source, const char *filename) {
    ...
    l->source = source; l->source_len = strlen(source);
    l->position = 0; l->line = 1; l->column = 1; l->filename = filename;
    return l;
}
...
void token_free(Token *t) {
    if (!t) return;
    ocl_free(t->lexeme);
    if (t->type == TOKEN_STRING || t->type == TOKEN_CHAR) ocl_free(t->value.string_value);
}
...
Token *lexer_tokenize_all(Lexer *l, size_t *out_count) {
    ...
    for (;;) {
        if (count >= cap) { cap *= 2; tokens = ocl_realloc(tokens, cap * sizeof(Token)); }
