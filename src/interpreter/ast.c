...
ASTNode *ast_create_if_stmt(SourceLocation loc, ExprNode *cond,
                             BlockNode *then_b, BlockNode *else_b) {
...
ExprNode *ast_create_binary_op(SourceLocation loc, ExprNode *left,
    ...
    n->left          = left;
    n->right         = right;
    n->operator      = op;   /* string literals – no ownership */
    return (ExprNode *)n;
}
...
void ast_free(ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case AST_INDEX_ACCESS: {
            ...
            ast_free((ASTNode *)e->index_access.array);
            ast_free((ASTNode *)e->index_access.index);
            break;
        }
        default: break;
    }
